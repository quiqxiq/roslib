...
// Entry point:
//
//   - roslib.NewFromConfig(ctx, cfg, log)  — single router via Manager
//   - roslib.NewFleet(ctx, fleetCfg, log)  — multi router via Manager
//   - roslib.NewManager()                  — Manager kosong, Register manual
//   - roslib.New(ctx, Options{...})        — konstruktor low-level satu device
//
// Dari Device (acquire lewat mgr.Get(...)), chain fluent device.Path(p)...
// atau register poll/stream langsung lewat RegisterPoll / RegisterStream.
package roslib
...
// DefaultDeviceKey adalah key yang dipakai NewFromConfig saat register
// single-router. Caller pakai ini untuk acquire device:
//
...
// satu-satu via mgr.Register(ctx, name, opts), atau pakai NewFromConfig /
// NewFleet untuk sekali setup.
...
//
// Untuk service production lebih baik pakai Manager (NewFromConfig /
// NewFleet) supaya device dapat re-acquire by-name tanpa re-dial.
func New(parent context.Context, opts Options) (*Device, error) {
	return device.New(parent, opts)
}

// NewFromConfig membangun Manager terisi 1 RouterDevice dari Config
...
func NewFromConfig(parent context.Context, cfg *Config, log *logrus.Logger) (*Manager, *influxdb3.Client, error) {
	if cfg == nil {
...
// NewFleet membangun Manager terisi semua RouterDevice dari FleetConfig
// (multi-router mode). Setiap entry di-register dengan key = entry.ID.
// Cache + influx client di-share antar device.
//
...
// Pola pakai:
//
//	mgr, influxCli, err := roslib.NewFleet(ctx, fleetCfg, log)
//	if err != nil { return err }
//	defer mgr.CloseAll()
...
//	    // ...
//	}
func NewFleet(parent context.Context, cfg *config.FleetConfig, log *logrus.Logger) (*Manager, *influxdb3.Client, error) {
	if cfg == nil {
